...
const SwarmInvestCard = ({ pool, className }: SwarmInvestCardProps) => {
  ...
  const { poolAccount, purchaseShares } = useLaunchpadProgramAccount({
    poolAddress: pool,
  });
  ...
  useEffect(() => {
    ...
    setData({
      ...
      pricePerShare: calculateSharePrice(
        poolAccount.data?.totalShares.toNumber() -
          poolAccount.data?.availableShares.toNumber()
      ),
  ...
  return (
    <Card className={cn("w-full", className)}>
      <div className="w-full flex flex-row justify-between items-center">
        ...
        <div className="flex flex-row items-center gap-1">
          <p className="text-muted">
            Available:{" "}
            <strong>
              {IntlNumberFormatCompact((data.remainingSupply * 10) || 0)}
            </strong>
          </p>
          <div className="text-xs text-yellow-500 ml-2">
            Only 10% of total supply available
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col basis-1/2 md:max-w-[48%]">
          <div className="border border-border rounded-md bg-card p-4 pb-2 flex flex-col">
            ...
            <div className="flex flex-row">
                ...
                style={{
                  width: `calc(${
                    data.remainingSupply !== 0
                      ? IntlNumberFormat(numShares).length || 1
                      : 8
                  }ch + ${connected ? 1 : 2}rem)`,
              ...
              />
              {data.remainingSupply !== 0 && (
                <p className="mt-auto mb-4" ref={sharesRef}>
                  / Shares
                </p>
              )}
            </div>
          ...
          <div className="flex flex-row justify-between items-center px-4 mt-2">
            <p className="text-sm text-muted">
              1 share = {IntlNumberFormat(data.pricePerShare)} $COMPUTE
            </p>
            <p className="text-sm text-muted">
              Total supply: {IntlNumberFormatCompact(data.totalSupply)}
            </p>
          </div>
        </div>
        <div className="flex flex-col basis-1/2 md:max-w-[48%]">
          ...
          <p className="text-sm text-muted pl-4 mt-2">
            Minimum: {IntlNumberFormat(data.pricePerShare)} $COMPUTE
          </p>
        </div>
      </div>
      {fee > 0 && (
      ...
      {!connected && data.remainingSupply !== 0 && (
        <ConnectButton className="mt-10 w-full md:max-w-40" />
      )}
    </Card>
  );
};
